...
@section('content')
    ...
    <div class="d-md-flex d-block justify-content-between align-items-center mb-3">
        ...
        <a href="{{ Route('demans.create') }}" class="btn btn-primary">Tambah Data</a>
    </div>
    @if (session('success'))
                ...
                <tbody>
                    @foreach ($demans as $demands)
                        <tr>
                            ...
                            <td>
                                <a href="{{ Route('demans.edit', $demands->id) }}" class="btn btn-light border btn-sm text-decoration-none mb-lg-0 mb-2">Update</a>
                                <span class="text-muted d-lg-inline d-none">|</span>
                                <form action="{{ Route('demans.delete', $demands->id) }}" method="POST" class="d-inline">
                                    @csrf
                                    {{ method_field('DELETE') }}
